...
reversed_new(PyTypeObject *type, PyObject *args, PyObject *kwargs)
{
    ...
    if (!_PyArg_CheckPositional("reversed", PyTuple_Size(args), 1, 1)) {
        goto exit;
    }
    seq = PyTuple_GetItem(args, 0);
...
enum_next_long(enumobject *en, PyObject* next_item)
{
    ...
    if (Py_REFCNT(result) == 1) {
        Py_INCREF(result);
        old_index = PyTuple_GetItem(result, 0);
        old_item = PyTuple_GetItem(result, 1);
        PyTuple_InitItem(result, 0, next_index);
        PyTuple_InitItem(result, 1, next_item);
        Py_DECREF(old_index);
        Py_DECREF(old_item);
        return result;
    }
