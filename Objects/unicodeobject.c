...
int PyString_CheckExact(PyObject *op) {
  return Py_IS_TYPE(op, &PyString_Type);
}
...
Py_ssize_t PyString_Size(PyObject *s) {
  return PyUnicode_GET_SIZE(s);
}

Py_hash_t PyString_Hash(PyObject *s) {
  return ((PyUnicodeObject *) s)->hash;
}
...
int PyString_CheckInterned(PyObject *s) {
  return PyUnicode_CHECK_INTERNED(s);
}
...
/* Py_DEPRECATED(3.3) */ PyAPI_FUNC(Py_UCS1) _PyUnicode_ToLowercase(
    Py_UCS1 ch       /* Unicode character */
    );

/* Py_DEPRECATED(3.3) */ PyAPI_FUNC(Py_UCS1) _PyUnicode_ToUppercase(
    Py_UCS1 ch       /* Unicode character */
    );

Py_DEPRECATED(3.3) PyAPI_FUNC(Py_UCS1) _PyUnicode_ToTitlecase(
    Py_UCS1 ch       /* Unicode character */
    );
...
#  define _PyUnicode_CHECK(op) PyString_Check(op)
...
/* Forward declaration */
static inline int
_PyStringWriter_WriteCharInline(_PyStringWriter *writer, Py_UCS1 ch);
...
/* forward */
static PyUnicodeObject *_PyString_New(Py_ssize_t length);
...
static PyObject *
unicode_center_impl(PyObject *self, Py_ssize_t width, Py_UCS1 fillchar);
...
static PyObject *
unicode_ljust_impl(PyObject *self, Py_ssize_t width, Py_UCS1 fillchar);
...
static PyObject *
unicode_rjust_impl(PyObject *self, Py_ssize_t width, Py_UCS1 fillchar);
...
_PyString_New(Py_ssize_t length)
{
    ...
    if (length < 0) {
        PyErr_SetString(PyExc_SystemError,
                        "Negative size passed to _PyString_New");
        return NULL;
    }

    unicode = PyObject_New(PyUnicodeObject, &PyString_Type);
    if (unicode == NULL)
        return NULL;
...
PyString_New(Py_ssize_t size)
{
    ...
    if (size < 0) {
        PyErr_SetString(PyExc_SystemError,
                        "Negative size passed to PyString_New");
        return NULL;
    }
    ...
    if (obj == NULL)
        return PyErr_NoMemory();
    obj = PyObject_INIT(obj, &PyString_Type);
    if (obj == NULL)
        return NULL;
                 ...
                 Py_ssize_t how_many)
{
    ...
    assert(0 <= from_start);
    assert(0 <= to_start);
    assert(PyString_Check(from));
    assert(from_start + how_many <= PyUnicode_GET_LENGTH(from));

    assert(PyString_Check(to));
                         ...
                         Py_ssize_t how_many)
{
    ...
    if (!PyString_Check(from) || !PyString_Check(to)) {
...
PyString_Resize(PyObject **p_unicode, Py_ssize_t length)
{
    ...
    unicode = *p_unicode;
    if (unicode == NULL || !PyString_Check(unicode) || length < 0)
    {
        PyErr_BadInternalCall();
        return -1;
    }
...
get_latin1_char(unsigned char ch)
{
    ...
    unicode = PyString_New(1);
    if (!unicode) {
        return NULL;
    }

    ((char *) PyUnicode_DATA(unicode))[0] = ch;
...
PyObject *
PyString_FromStringAndSize(const char *u, Py_ssize_t size)
{
    if (size < 0) {
        PyErr_SetString(PyExc_SystemError,
    ...
    if (u != NULL)
      return _PyUnicode_FromUCS1((const unsigned char *)u, size);
        ...
        return (PyObject *)_PyString_New(size);
}

PyObject *
PyString_FromString(const char *u)
{
...
_PyUnicode_FromId(_Py_Identifier *id)
{
    ...
    PyObject *obj;
    obj = PyString_FromStringAndSize(id->string,
				      strlen(id->string));
    if (!obj) {
        return NULL;
    }
...
_PyUnicode_FromUCS1(const Py_UCS1* u, Py_ssize_t size)
{
    ...
    if (!res)
        return NULL;
    memcpy(PyUnicode_DATA(res), u, size);
    return res;
}
...
static int
unicode_fromformat_write_str(_PyStringWriter *writer, PyObject *str,
                             Py_ssize_t width, Py_ssize_t precision)
{
    ...
    if ((precision == -1 || precision >= length)
        && width <= length)
        return _PyStringWriter_WriteStr(writer, str);
    ...
    if (_PyStringWriter_Prepare(writer, arglen) == -1)
        return -1;
...
static int
unicode_fromformat_write_cstr(_PyStringWriter *writer, const char *str,
                              Py_ssize_t width, Py_ssize_t precision)
{
    /* UTF-8 */
    Py_ssize_t length;
    ...
    unicode = PyString_FromStringAndSize(str, length);
    if (unicode == NULL)
        return -1;
...
static const char*
unicode_fromformat_arg(_PyStringWriter *writer,
                       const char *f, va_list *vargs)
{
    ...
    case 'c':
    {
        ...
        if (_PyStringWriter_WriteCharInline(writer, ordinal) < 0)
            return NULL;
        break;
    }
    ...
    case '%':
        if (_PyStringWriter_WriteCharInline(writer, '%') < 0)
            return NULL;
        break;
...
PyString_FromOrdinal(int ordinal)
{
    ...
    return unicode_char((Py_UCS1)ordinal);
}
...
const char *
PyString_AsCharAndSize(PyObject *unicode, Py_ssize_t *psize)
{
  if (psize)
...
const char *
PyString_AsChar(PyObject *unicode)
{
    return PyString_AsCharAndSize(unicode, NULL);
}
...
int
PyUnicode_WriteChar(PyObject *unicode, Py_ssize_t index, Py_UCS1 ch)
{
  if (!PyString_Check(unicode)) {
        PyErr_BadArgument();
        return -1;
...
/* --- 7-bit ASCII Codec -------------------------------------------------- */
PyObject *
_PyString_TransformDecimalAndSpaceToASCII(PyObject *unicode)
{
    if (!PyString_Check(unicode)) {
        PyErr_BadInternalCall();
        return NULL;
    ...
    PyObject *result = PyString_New(len);
    if (result == NULL) {
        return NULL;
    }

    Py_UCS1 *out = PyUnicode_DATA(result);
               ...
               int direction)
{
    ...
    if (len2 == 1) {
        Py_UCS1 ch = PyUnicode_READ(buf2, 0);
...
Py_ssize_t
PyString_FindChar(PyObject *str, Py_UCS1 ch,
                   ...
                   int direction)
{
    ...
    if (end - start < 1)
        return -1;
    result = findchar(PyUnicode_DATA(str) + start,
                      end-start, ch, direction);
    if (result == -1)
        return -1;
    else
...
static Py_ssize_t
do_upper_or_lower(const void *data, Py_ssize_t length, Py_UCS1 *res,
                  int lower)
{
    Py_ssize_t i, k = 0;
...
static Py_ssize_t
do_upper(const void *data, Py_ssize_t length, Py_UCS1 *res)
{
...
static Py_ssize_t
do_title(const void *data, Py_ssize_t length, Py_UCS1 *res)
{
    Py_ssize_t i, k = 0;
...
case_operation(PyObject *self,
               Py_ssize_t (*perform)(const void *, Py_ssize_t, Py_UCS1 *))
{
    PyObject *res = NULL;
    ...
    if ((size_t) length > PY_SSIZE_T_MAX / (3 * sizeof(Py_UCS1))) {
        PyErr_SetString(PyExc_OverflowError, "string is too long");
        return NULL;
    }
    tmp = PyMem_Malloc(sizeof(Py_UCS1) * 3 * length);
    if (tmp == NULL)
        return PyErr_NoMemory();
    newlength = perform(data, length, tmp);
    res = PyString_New(newlength);
    if (res == NULL)
        goto leave;
    tmpend = tmp + newlength;
    outdata = PyUnicode_DATA(res);
    _PyUnicode_CONVERT_BYTES(Py_UCS1, Py_UCS1, tmp, tmpend, outdata);
  leave:
    PyMem_Free(tmp);
    return res;
}
...
_PyString_JoinArray(PyObject *separator, PyObject *const *items, Py_ssize_t seqlen)
{
    ...
    last_obj = NULL;
    if (seqlen == 1) {
        if (PyString_CheckExact(items[0])) {
            res = items[0];
            Py_INCREF(res);
            return res;
        }
    ...
    if (use_memcpy) {
        ...
        assert(res_data == PyUnicode_DATA(res)
    ...
    unsigned char fill)
{
    ...
    u = PyString_New(left + _PyUnicode_LENGTH(self) + right);
    if (!u)
        return NULL;
...
{
    Py_UCS1 *fillcharloc = (Py_UCS1 *)addr;
...
{
    if (PyString_Check(left) && PyString_Check(right)) {
...
int
_PyString_EqualToASCIIString(PyObject *unicode, const char *str)
{
    size_t len;
    assert(_PyUnicode_CHECK(unicode));
    assert(str);
    ...
    return strlen(str) == len &&
           memcmp(PyUnicode_DATA(unicode), str, len) == 0;
}

int
_PyString_EqualToASCIIId(PyObject *left, _Py_Identifier *right)
{
    PyObject *right_uni;
    ...
    if (right_uni == NULL) {
        ...
        PyErr_Clear();
        return _PyString_EqualToASCIIString(left, right->string);
    }
...
PyString_Append(PyObject **p_left, PyObject *right)
{
    ...
    if (right == NULL || left == NULL
        || !PyString_Check(left) || !PyString_Check(right)) {
        if (!PyErr_Occurred())
            PyErr_BadInternalCall();
        goto error;
    }
...
PyString_Substring(PyObject *self, Py_ssize_t start, Py_ssize_t end)
{
    ...
    length = end - start;
      {
        data = PyUnicode_DATA(self);
        return PyString_FromStringAndSize((char *) (data + start), length);
    }
}
...
unicode_repeat(PyObject *str, Py_ssize_t len)
{
    ...
    u = PyString_New(nchars);
    if (!u)
        return NULL;

    if (PyUnicode_GET_LENGTH(str) == 1) {
        Py_UCS1 fill_char = PyUnicode_READ( PyUnicode_DATA(str), 0);
...
unicode_repr(PyObject *unicode)
{
    ...
    squote = dquote = 0;
    for (i = 0; i < isize; i++) {
        Py_UCS1 ch = PyUnicode_READ(idata, i);
        Py_ssize_t incr = 1;
        switch (ch) {
        case '\'': squote++; break;
        case '"':  dquote++; break;
...
static inline void
_PyStringWriter_Update(_PyStringWriter *writer)
{
                                 ...
                                 Py_ssize_t length)
{
    ...
    /* ensure that the _PyStringWriter_Prepare macro was used */
    ...
    if (writer->buffer == NULL) {
        ...
        writer->buffer = PyString_New(newlen);
        if (writer->buffer == NULL)
            return -1;
    }
...
_PyStringWriter_WriteCharInline(_PyStringWriter *writer, Py_UCS1 ch)
{
    ...
    if (_PyStringWriter_Prepare(writer, 1) < 0)
        return -1;
...
int
_PyStringWriter_WriteChar(_PyStringWriter *writer, Py_UCS1 ch)
{
    return _PyStringWriter_WriteCharInline(writer, ch);
}

int
_PyStringWriter_WriteStr(_PyStringWriter *writer, PyObject *str)
{
    Py_ssize_t len;
...
int
_PyStringWriter_WriteSubstring(_PyStringWriter *writer, PyObject *str,
                                Py_ssize_t start, Py_ssize_t end)
{
    Py_ssize_t len;
    assert(0 <= start);
    ...
    if (_PyStringWriter_Prepare(writer, len) < 0)
        return -1;
...
/*[clinic end generated code: output=45fceaca6d2ba4c8 input=5e135645d167a214]*/
{
    ...
    _PyStringWriter_Init(&writer);
    ret = _PyString_FormatAdvancedWriter(&writer,
                                          self, format_spec, 0,
...
unicode_subscript(PyObject* self, PyObject* item)
{
    ...
    if (_PyIndex_Check(item)) {
        ...
        /* General case */
        src_data = PyUnicode_DATA(self);
        result = PyString_New(slicelength);
        if (result == NULL)
            return NULL;
        dest_data = PyUnicode_DATA(result);

        for (cur = start, i = 0; i < slicelength; cur += step, i++) {
            Py_UCS1 ch = PyUnicode_READ(src_data, cur);
            ...
            _PyStringWriter *writer)
{
    ...
    if (writer) {
        ...
        *p_output = PyString_FromStringAndSize(p, len);
    PyMem_Free(p);
    return 0;
}
...
_PyUnicode_FormatLong(PyObject *val, int alt, int prec, int type)
{
    ...
    if (prec > numdigits) {
        ...
        *b1 = '\0';
        Py_DECREF(result);
        result = r1;
        buf = (char *) PyString_AsChar(result);
    ...
    if (!PyString_Check(result)
        || buf != PyUnicode_DATA(result)) {
        PyObject *unicode;
        unicode = PyString_FromStringAndSize(buf, len);
        Py_DECREF(result);
        result = unicode;
    }
...
{
    PyObject *v;
    _PyStringWriter *writer = &ctx->writer;
    ...
    case 'a':
        ...
        if (PyString_CheckExact(v) && arg->ch == 's') {
            *p_str = v;
            Py_INCREF(*p_str);
        }
        else {
    ...
    case 'c':
    {
        Py_UCS1 ch = formatchar(v);
        if (ch == (Py_UCS1) -1)
            return -1;
        if (arg->width == -1 && arg->prec == -1) {
            /* Fast path */
            if (_PyStringWriter_WriteCharInline(writer, ch) < 0)
                return -1;
            return 1;
        }
                          ...
                          PyObject *str)
{
    ...
    {
        /* Fast path */
        if (_PyStringWriter_WriteStr(writer, str) == -1)
            return -1;
        return 0;
    }
    ...
    signchar = '\0';
    if (arg->sign) {
        Py_UCS1 ch = PyUnicode_READ( pbuf, pindex);
        if (ch == '-' || ch == '+') {
            signchar = ch;
            len--;
            pindex++;
    ...
    if (arg->sign && len == arg->width)
        buflen++;
    if (_PyStringWriter_Prepare(writer, buflen) == -1)
        return -1;
...
unicode_format_arg(struct unicode_formatter_t *ctx)
{
    ...
    if (arg.ch == '%') {
        ctx->fmtpos++;
        ctx->fmtcnt--;
        if (_PyStringWriter_WriteCharInline(&ctx->writer, '%') < 0)
            return -1;
        return 0;
    }
    arg.flags = 0;
...
PyString_Format(PyObject *format, PyObject *args)
{
    ...
    _PyStringWriter_Init(&ctx.writer);
    ...
    ctx.args_owned = 0;
    if (PyMapping_Check(args) && !PyTuple_Check(args) && !PyString_Check(args))
        ctx.dict = args;
    else
        ctx.dict = NULL;
    ctx.args = args;
    ...
    while (--ctx.fmtcnt >= 0) {
        if (PyUnicode_READ(ctx.fmtdata, ctx.fmtpos) != '%') {
            ...
            if (_PyStringWriter_WriteSubstring(&ctx.writer, ctx.fmtstr,
    ...
    return _PyStringWriter_Finish(&ctx.writer);

  onError:
    _PyStringWriter_Dealloc(&ctx.writer);
    if (ctx.args_owned) {
        Py_DECREF(ctx.args);
    }
    return NULL;
...
unicode_subtype_new(PyTypeObject *type, PyObject *args, PyObject *kwds)
{
    ...
    unicode = unicode_new(&PyString_Type, args, kwds);
    if (unicode == NULL)
        return NULL;
    assert(_PyUnicode_CHECK(unicode));
...
unicodeiter_next(unicodeiterobject *it)
{
    ...
    if (it->it_index < PyUnicode_GET_LENGTH(seq)) {
        ...
        Py_UCS1 chr = PyUnicode_READ(data, it->it_index);
...
unicodeiter_reduce(unicodeiterobject *it, PyObject *Py_UNUSED(ignored))
{
    ...
    if (it->it_seq != NULL) {
        ...
        PyObject *u = (PyObject *)_PyString_New(0);
        if (u == NULL)
            return NULL;
...
unicode_iter(PyObject *seq)
{
    ...
    it = PyObject_New(unicodeiterobject, &PyStringIter_Type);
    if (it == NULL)
        return NULL;
    it->it_index = 0;
    Py_INCREF(seq);
                                ...
                                const char **first_invalid_escape)
{
    ...
    const char *end;
    PyObject *temp;
    temp = PyString_New(len);
    p = (char *) PyString_AsChar(temp);
    ...
    *first_invalid_escape = NULL;
    ...
    PyString_Resize(&temp, (p - PyString_AsChar(temp)));
    return temp;
