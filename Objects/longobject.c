...
int PyLong_Check(PyObject *op) {
  return PyType_HasFeature(Py_TYPE(op), Py_TPFLAGS_LONG_SUBCLASS);
}
...
long_new(PyTypeObject *type, PyObject *args, PyObject *kwargs)
{
    ...
    fastargs = _PyArg_UnpackKeywords(PyTuple_Items(args), nargs, kwargs, NULL, &_parser, 0, 2, 0, argsbuf);
    if (!fastargs) {
        goto exit;
    }
    if (nargs < 1) {
...
_PyLong_New(Py_ssize_t size)
{
    ...
    result = PyMem_Malloc(offsetof(PyLongObject, ob_digit) +
                             size*sizeof(digit));
    if (!result) {
        PyErr_NoMemory();
        return NULL;
...
#define CHECK_BINOP(v,w)                                \
    do {                                                \
      if (!PyLong_Check((PyObject *) v) || !PyLong_Check((PyObject *) w)) \
                                ...
                                char **bytes_str)
{
    ...
    a = (PyLongObject *)aa;
    if (a == NULL || !PyLong_Check((PyObject *)a)) {
        PyErr_BadInternalCall();
        return -1;
    }
...
#define WRITE_UNICODE_DIGITS(TYPE)                                    \
    do {                                                              \
        if (writer)                                                   \
            p = (TYPE*)PyString_AsChar(writer->buffer) + writer->pos + strlen; \
        else                                                          \
            p = (TYPE*)PyString_AsChar(str) + strlen;                  \
                                                                      \
        WRITE_DIGITS(p);                                              \
                                                                      \
        /* check we've counted correctly */                           \
        if (writer)                                                   \
            assert(p == ((TYPE*)PyString_AsChar(writer->buffer) + writer->pos)); \
        else                                                          \
            assert(p == (TYPE*)PyString_AsChar(str));                  \
    } while (0)
...
#define WRITE_UNICODE_DIGITS(TYPE)                                      \
    do {                                                                \
        if (writer)                                                     \
            p = (TYPE*)PyString_AsChar(writer->buffer) + writer->pos + sz; \
        else                                                            \
            p = (TYPE*)PyString_AsChar(v) + sz;                          \
                                                                        \
        WRITE_DIGITS(p);                                                \
                                                                        \
        if (writer)                                                     \
            ...
            assert(p == (TYPE*)PyString_AsChar(v));                      \
    } while (0)
    {
        Py_UCS1 *p;
        WRITE_UNICODE_DIGITS(Py_UCS1);
