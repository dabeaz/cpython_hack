...
float___getformat__(PyTypeObject *type, PyObject *arg)
{
    ...
    typestr = PyString_AsCharAndSize(arg, &typestr_length);
    if (typestr == NULL) {
        goto exit;
    }
...
float___set_format__(PyTypeObject *type, PyObject *const *args, Py_ssize_t nargs)
{
    ...
    Py_ssize_t typestr_length;
    typestr = PyString_AsCharAndSize(args[0], &typestr_length);
    if (typestr == NULL) {
        goto exit;
    }
    if (strlen(typestr) != (size_t)typestr_length) {
    ...
    fmt = PyString_AsCharAndSize(args[1], &fmt_length);
    if (fmt == NULL) {
        goto exit;
    }
...
PyFloat_FromString(PyObject *v)
{
    ...
    if (PyString_Check(v)) {
        ...
        s = PyString_AsCharAndSize(s_buffer, &len);
        assert(s != NULL);
    }
    else {
...
{
  if (PyFloat_CheckExact((PyObject*) op)) {
	PyMem_Free(op);
	return;
    } else {
...
/*[clinic end generated code: output=46c0274d22b78e82 input=0407bebd354bca89]*/
{
    ...
    s = PyString_AsCharAndSize(string, &length);
    if (s == NULL)
        return NULL;
    s_end = s + length;
